...
static CharsetTable *get_charset_table(void)
{
	...
	if (!ctable) {
		...
		ctable->num++;
		const gchar *charset[] = {"BIG5", "GB18030"};
		for (i=0; i<2; i++)
			if( g_strcasecmp(ctable->charset[ctable->num-1], charset[i]) == 0 ) {
				ctable->charset[ctable->num] = charset[i^1];
...
#if ENABLE_CHOOSER
#ifdef USE_HILDON
	selector = hildon_file_chooser_dialog_new (GTK_WINDOW (pub->mw->window),
